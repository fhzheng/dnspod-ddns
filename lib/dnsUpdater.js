...
function sendMessage(domain, oldIp, ip, callback) {
    messageService.getService("message/xyz.zpath.java.mqdemo.proto.Email").then(service => {
        logger.debug(service.name, 'service loaded');
        var current_time = require('moment')();
        var message = {
            from: 'sender@domain.host',
            to: 'receiver@domain.host',
            title: domain + "'s IP address has changed! <" + current_time.format('YYYY-MM-DD HH:mm:ss') + ">",
            body: 'Current IP is ' + ip + '. Old IP is ' + oldIp,
        };
...
DNSUpdater.prototype.updateRecordValue = function () {
    ...
    async.waterfall([
        ...
        function (result, callback) {
            var logInfo = util.format('Old ip address is %j, while new ip address is %j', oldRecordValue, result);
        ...
        function (callback, error) {
            if (error) {
                ...
                sendMessage(self.params.sub_domain + '.' + self.params.domain, oldRecordValue, self.params.record_value, error => {
                    if (error) {
                        callback(error);
                    } else {
                        callback(null);
